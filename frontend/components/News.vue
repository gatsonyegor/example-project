        ...
        <div class="card">
            <div class="font-semibold text-xl mb-4">
                ...
                <div v-else class="flex flex-col">
                    ...
                    >
                        <template #title>
                            ...
                            >
                                ...
                                <div
                                    class="ml-4 flex flex-col items-end text-xs text-gray-500"
                                >
                                    <span v-if="!loading">{{
                                        item.source
                                    }}</span>
                    ...
                    <p v-else class="text-center">Ничего не найдено</p>
                </div>
...
async function loadNews(page: number = 1): Promise<void> {
    ...
    try {
        const response = await fetch(
            `${config.public.apiBase}/api/news?page=${page}&limit=${rows.value}&search=${search.value}`,
            {
                headers: {
